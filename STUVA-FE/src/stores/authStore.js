  ...
  actions: {
    async login(username, password) {
      ...
      try {
        ...
        const userRole =
          data.user?.role || data.role || this.detectRoleFromUsername(username)
    ...
    detectRoleFromUsername(username) {
      ...
      if (username.startsWith('08')) {
        ...
        username.toLowerCase().includes('ortu') ||
        username.toLowerCase().includes('parent')
      ) {
        console.log('→ Detected: Orang Tua (keyword match)')
        return 'ortu'
      } else if (
        username.toLowerCase().includes('guru') ||
      ...
      try {
        ...
        if (this.token) {
          try {
            ...
            console.warn(
              '⚠️ Logout API failed (continuing with local logout):',
              {
