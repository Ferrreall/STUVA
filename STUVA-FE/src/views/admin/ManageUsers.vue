          ...
          >
            <div class="user-identity">
              ...
              <div class="user-main">
                ...
                <p class="user-username">{{ usernameLabel }}: {{ u.username || '-' }}</p>
              </div>
            </div>
...
// Props dari route (lihat konfigurasi router di bawah)
const props = defineProps({
  role: { type: String, required: true },   // 'siswa' | 'guru' | 'ortu'
...
const formFields = computed(() => {
  const fields = [
    ...
    { key: 'username', label: usernameLabel.value, type: 'text', required: true },
    { key: 'email', label: 'Email', type: 'email' },
    { key: 'phone', label: 'No. HP', type: 'tel' }
  ]
  if (props.role === 'siswa') {
    fields.push({ key: 'class_name', label: 'Kelas', type: 'text', placeholder: 'cth: XII RPL 1' })
  }
  if (props.role === 'guru') {
    fields.push({ key: 'subject', label: 'Mata Pelajaran', type: 'text', placeholder: 'cth: Matematika' })
  }
  if (!isEditing.value) {
...
// ===== Fetch jumlah total dari /users/stats =====
const fetchStats = async () => {
  try {
    console.log('📡 Fetching user stats (manage)...')
    ...
    let count = null
      ...
      // objek: { total_siswa, total_guru, total_ortu }
      const map = {
    ...
    console.log(`✅ Total ${roleLabel.value}:`, totalCount.value)
  } catch (error) {
    console.error('❌ Error fetching stats:', error)
    totalCount.value = null  // badge disembunyiin kalau gagal
  }
}
...
const fetchUsers = async () => {
  ...
  try {
    ...
    // filter by role sesuai halaman
    users.value = items.filter(u => normalizeRole(u.role) === props.role)

    console.log(`✅ ${users.value.length} ${roleLabel.value} loaded`)

        // sinkronkan total dari hasil filter (biar badge akurat walau stats API telat)
...
const openCreate = () => {
  ...
  formData.value = { name: '', username: '', email: '', phone: '', password: '' }
...
const submitForm = async () => {
  ...
  try {
    ...
    // password kosong saat edit → jangan kirim (biar tidak ke-reset)
    if (isEditing.value && !payload.password) {
      delete payload.password
    }

    if (isEditing.value) {
      // API kamu pakai POST untuk update
